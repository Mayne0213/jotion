...
print_usage() {
    ...
    echo "  -n, --namespace   Kubernetes 네임스페이스 (기본: jotion)"
    echo "  -c, --context     kubectl 컨텍스트 지정 (kubectl config current-context 기본)"
    echo "      --app-image   app 디플로이먼트에 사용할 이미지 레퍼런스(예: your/repo:jotion)"
    echo "      --build       배포 전에 Dockerfile로 로컬 이미지를 빌드 (기본)"
    echo "      --no-build    빌드 생략"
    echo "      --tag         --build 시 사용할 이미지 태그 (기본: jotion-app:local-<timestamp>)"
    echo "      --dry-run     실제 적용 대신 미리보기 수행"
    echo ""
}

# 기본값
K8S_NAMESPACE="jotion"
KUBE_CONTEXT=""
DRY_RUN="false"
APP_IMAGE=""
DO_BUILD="true"
...
if [[ "${DO_BUILD}" == "true" && "${DRY_RUN}" != "true" ]]; then
    BUILD_CONTEXT="services/nextjs"
    DOCKERFILE_PATH="deploy/docker/Dockerfile.prod"
    if [[ -z "${IMAGE_TAG}" ]]; then
        IMAGE_TAG="jotion-app:local-$(date +%Y%m%d%H%M%S)"
    fi
...
if [[ -n "${APP_IMAGE}" ]]; then
    ...
    kubectl -n "${K8S_NAMESPACE}" set image deployment/app jotion-app="${APP_IMAGE}"
    # latest 태그나 기본 정책으로 인한 강제 Pull 방지
    log_info "🛠️ 이미지 Pull 정책 패치: IfNotPresent"
    kubectl -n "${K8S_NAMESPACE}" patch deployment app \
      --type='json' \
...
echo "  - 로그: kubectl logs deploy/app -n ${K8S_NAMESPACE} -f"
echo "  - 상세: kubectl describe deploy/app -n ${K8S_NAMESPACE}"

# kubectl port-forward -n jotion deploy/app 3000:3000
