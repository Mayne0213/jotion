...
print_usage() {
    ...
    echo "  -n, --namespace        정리할 네임스페이스 (기본: jotion)"
    echo "      --context          kubectl 컨텍스트 지정"
    echo "      --delete-namespace 네임스페이스 자체를 삭제 (주의)"
    echo "      --delete-images    로컬 Docker의 jotion 관련 이미지 삭제"
...
if [[ "${DELETE_NAMESPACE}" == "true" ]]; then
    ...
    # PVC 및 PV 삭제 (참고: DB는 Supabase 사용으로 데이터 손실 없음)
...
if [[ "${DELETE_IMAGES}" == "true" ]]; then
    log_info "🧽 로컬 Docker 이미지 정리: jotion-app 관련 태그"
    docker images --format '{{.Repository}}:{{.Tag}}' | grep -E '^jotion-app:local|^jotion-app:' | xargs -r docker rmi -f 2>/dev/null || true
fi
